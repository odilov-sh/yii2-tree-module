...
;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.init = function () {
    ...
    this._elemArrowBtn = $('<div id="' + this.comboTreeId + 'ArrowBtn" class="comboTreeArrowBtn" type="button"  style="width: 30px;"><span class="glyphicon glyphicon-chevron-down comboTreeArrowBtnImg" style="padding-left: 8px; padding-top: 10px;"></span></div>');
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    var itemHtml = "",
      isThereSubs = sourceItem.hasOwnProperty("subs") &&  sourceItem.subs.length;
    ...
    itemHtml += '<LI id="' + this.comboTreeId + 'Li' + sourceItem.id + '" class="ComboTreeItem' + (isThereSubs?'Parent':'Chlid') + '" style="padding-left: 20px"> ';


    if (isThereSubs)
      itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '<span class="glyphicon glyphicon-chevron-right"></span>' : '<span class="glyphicon glyphicon-chevron-down"></span>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined){
      if ($(subMenu).is(':visible'))
        $(item).children('span.comboTreeParentPlus').html('<span class="glyphicon glyphicon-chevron-right"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
      else
        $(item).children('span.comboTreeParentPlus').html('<span class="glyphicon glyphicon-chevron-down"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
  ...
  ComboTree.prototype.refreshInputVal = function () {
    ...
    if (this.options.isMultiple) {
      ...
      tmpIds = this._selectedItem.id;
    }

    this._elemInput.val(tmpTitle);

    if (this.options.hiddenInput){
      this.options.hiddenInput.val(tmpIds)
    }
